"""
Ejecuta el orquestador sobre lista95.pdf y reporta uso de tokens + costo estimado.
Uso: python tests/run_with_usage.py [ANTHROPIC_API_KEY]
"""
...
# Agrega pricebot/api al path para importar main.py directamente
...
# API key opcional via argumento CLI para evitar problemas con el env de la shell
if len(sys.argv) > 1 and sys.argv[1].strip():
    os.environ["ANTHROPIC_API_KEY"] = sys.argv[1].strip()

# Tarifas por defecto (USD por 1M tokens) para el tier Claude Sonnet
# Se pueden sobreescribir con env vars INPUT_RATE_PER_M / OUTPUT_RATE_PER_M
...
async def tracked_claude_chat(messages: list, system: str = "", max_tokens: int = 8000) -> str:
    ...
    texts = [b["text"] for b in content_blocks if isinstance(b, dict) and b.get("text")]
...
async def run() -> None:
    ...
    pdf = Path(__file__).parent / "samples" / "lista95.pdf"
    if not pdf.exists():
    ...
    out_file = Path(__file__).parent / "outputs" / "orchestrator_usage_summary.json"
