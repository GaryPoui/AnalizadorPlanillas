"""
Extracción híbrida LCT: heurística existente + Claude por página.
Costo estimado: ~$0.09 USD (Haiku real) / ~$0.35 USD (tasas display Sonnet $3/$15 por M)

Guarda resultado en Respuestas/LCT...._hybrid.json (no sobreescribe el base).
"""
import asyncio
import json
import os
...
LCT_PDF       = REPO_ROOT / "Listas" / "LCT Lista de Precios 02-2026 (4).pdf"
HEURISTIC_JSON = REPO_ROOT / "Respuestas" / "LCT Lista de Precios 02-2026 (4).json"
OUTPUT_JSON   = REPO_ROOT / "Respuestas" / "LCT Lista de Precios 02-2026 (4)_hybrid.json"

load_dotenv(REPO_ROOT / "pricebot" / ".env", encoding="utf-8-sig")
API_KEY      = os.getenv("ANTHROPIC_API_KEY", "").strip()
CLAUDE_MODEL = os.getenv("CLAUDE_MODEL", "claude-haiku-4-5")

# Display rates (Sonnet reference used by the project)
...
SYSTEM_PROMPT = (
    ...
    '  "code": product code (4-5 digit number, or alphanumeric e.g. SCA-10, UCA-16, HM-12CB)\n'
    ...
    "\n"
    "Rules:\n"
    "- Extract ALL products, including from BOTH columns when a page has two side-by-side product columns\n"
...
async def _call_claude(client: httpx.AsyncClient, page_num: int, text: str) -> list[dict]:
    ...
    try:
        ...
        # Salvage truncated JSON array when max_tokens was hit
...
def _norm_price(val) -> str:
    ...
    # Remove any stray currency chars
    s = re.sub(r"[^\d.]", "", s)
    try:
...
def _is_valid_code(code: str) -> bool:
    if not code:
        return False
    # Accept 4-5 digit numeric OR alphanumeric codes (≥2 chars, starts with letter or digit)
    return bool(re.match(r"^(\d{4,5}|[A-Z][A-Z0-9\-/]{1,15})$", code.upper()))


async def main() -> None:
    if not API_KEY:
        print("ERROR: ANTHROPIC_API_KEY no configurada en pricebot/.env")
        sys.exit(1)

    print(f"=== Extracción híbrida LCT  (modelo: {CLAUDE_MODEL}) ===")
    print(f"PDF: {LCT_PDF.name}")
    print(f"Costo estimado: ~$0.09 USD (Haiku real) / ~$0.35 USD (tasas display)")
    print()

    # Cargar base heurística
    ...
    if HEURISTIC_JSON.exists():
        ...
        base_data = {}
    print(f"Base heurística cargada: {len(heuristic_rows)} filas")

    # Leer páginas del PDF
    with pdfplumber.open(LCT_PDF) as pdf:
        pages = [(i + 1, p.extract_text() or "") for i, p in enumerate(pdf.pages)]
    print(f"Páginas PDF: {len(pages)}")
    ...
    print(f"\n--- Claude listo en {elapsed}s ---")
    print(f"Tokens input: {_total_in:,}  output: {_total_out:,}  total: {_total_in + _total_out:,}")
    print(f"Gasto: ${total_cost:.4f} USD (tasas display Sonnet ${INPUT_RATE}/${OUTPUT_RATE} por M)")
    print()

    # Normalizar filas Claude al schema del proyecto
    LCT_CODE = "LCT"
    LCT_DESC = "LCT Lista de Precios 02-2026"
    ...
    print(f"Filas Claude válidas (code + precio): {len(claude_normalized)}")

    # Merge: heurística como base, Claude agrega y enriquece
    ...
    for row in claude_normalized:
        ...
        if code not in merged:
            ...
            # Enriquecer descripción si Claude tiene más información
    ...
    print()
    print(f"Filas heurística:              {len(heuristic_rows):>5}")
    print(f"Nuevas aportadas por Claude:   {new_from_claude:>5}")
    print(f"Descripciones enriquecidas:    {enriched:>5}")
    print(f"TOTAL final merged:            {len(final_rows):>5}")

    # Guardar en archivo separado (no sobreescribe el base)
